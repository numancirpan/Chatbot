...
EVALUATION_PATH = ROOT_DIR / "data" / "evaluation_cases.json"
...
def main() -> int:
    ...
    EVALUATION_PATH.write_text(json.dumps(cases, ensure_ascii=False, indent=2), encoding="utf-8")
    print(
        json.dumps(
            {"evaluation_cases": len(cases), "appended": len(appended), "updated": len(updated)},
            ensure_ascii=False,
            indent=2,
        )
    )
