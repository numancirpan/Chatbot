"""
Evaluate the chatbot against a small held-out evaluation question set.
...
def main() -> int:
    ...
    parser.add_argument("--output", help="Write machine-readable JSON results to this file.")
    parser.add_argument("--limit", type=int, default=0, help="Evaluate at most this many cases.")
    parser.add_argument("--start", type=int, default=0, help="Start index for partial evaluation.")
    ...
    try:
        ...
        for index, case in enumerate(cases, start=1):
            ...
            print(
                f"[{index}/{len(cases)}] {'PASS' if result['passed'] else 'FAIL'} {result['id']}",
                file=sys.stderr,
                flush=True,
            )
    except requests.exceptions.RequestException:
        print("Ollama erisilemiyor. Evaluation calistirmadan once `ollama serve` komutunu baslatin.")
        return 2
    passed_count = sum(1 for result in results if result["passed"])
    payload = {"passed": passed_count, "total": len(results), "results": results}
